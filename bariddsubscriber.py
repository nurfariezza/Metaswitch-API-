...
def job():
    ...
    try:
        ...
        l = []
        q = """
            select top 2000 m_id, accountid, directorynumber, allow from metaswitchbariddsubs where m_status = 0
            """
        ...
        q = """
            update metaswitchbariddsubs set m_status = 1 where m_id = ?
            """
        for s in l:
            db.cur.execute(q, s)
        ...
        q = """
            select top 2000 m_id, accountid, directorynumber,allow from metaswitchbariddsubs where status_comparing = 0
            """
        rows = db.cur.execute(q).fetchall()
        for r in rows:
            dn = r.directorynumber.strip()
            ...
            if h:
                l.append(r.m_id)
        q = """
            update metaswitchbariddsubs set status_comparing = ? where m_id = ?
            """
        for s in l:
