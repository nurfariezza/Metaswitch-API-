...
fileConfig('logging_resumesubscriber.conf')

logger = logging.getLogger(__name__)
requests.packages.urllib3.disable_warnings()

def resumesubs(accountid, dn):
    b = False
...
def job():
    ...
    try:
        ...
        l = []
        q = """
            select top 2000 m_id, accountid, directorynumber from metaswitchsuspendsubs where suspend = 0 and m_status = 0
            """
        rows = db.cur.execute(q).fetchall()
        for r in rows:
            dn = r.directorynumber.strip()
            b = resumesubs(r.accountid, dn)
            if b:
                l.append(r.m_id)
