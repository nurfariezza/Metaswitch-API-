...
fileConfig('logging_bariddpbx.conf')
...
def bariddsubs(dn, allow):
    ...
    try:
        u = geturltyped('api/pbx/lcc/{0}/{1}'.format(dn, allow))
...
def job():
    ...
    try:
        ...
        l = []
        q = """
            select top 2000 m_id, accountid, directorynumber, allow from metaswitchbariddpbx where m_status = 0
            """
        ...
        q = """
            update metaswitchbariddpbx set m_status = 1 where m_id = ?
            """
        for s in l:
            db.cur.execute(q, s)
        ...
        q = """
            select top 2000 m_id, accountid, directorynumber,allow from metaswitchbariddpbx where status_comparing = 0
            """
        rows = db.cur.execute(q).fetchall()
        for r in rows:
            dn = r.directorynumber.strip()
            ...
            if h:
                l.append(r.m_id)
        q = """
            update metaswitchbariddpbx set status_comparing = ? where m_id = ?
            """
        for s in l:
